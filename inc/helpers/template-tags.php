...
//Function for Showing Blog post date, Update date, Publish Date
function omnixima_posted_on(){
...
if (get_the_time( 'U') !== get_the_modified_time( 'U' )){
    ...
    $posted_on = sprintf(
        esc_html_x( 'Posted on %s', 'post date', 'omnixima' ),
...
function omnixima_posted_by(){
    $byline = sprintf(esc_html_x( ' by %s', 'post author', 'omnixima'),
...
function omnixima_the_excerpt( $trim_character_count = 0 ){
...
function omnixima_pagination(){
    ...
    printf( '<nav class="omnixima-pagination clearfix">%s</nav>', wp_kses(paginate_links( $args ), $allowed_tags ));
}
